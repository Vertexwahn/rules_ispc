def ispc_cc_library(name, out, ispc_main_source_file, srcs, target_compatible_with = [], **kwargs):
    ...
    native.genrule(
        ...
        cmd = select({
            ...
            "@bazel_tools//src/conditions:darwin_x86_64": "$(location @ispc_osx_x86_64//:ispc) --target=avx2 --target-os=macos --arch=x86-64 --pic $(locations %s) --header-outfile=$(location %s) -o $(location %s.o)" % (ispc_main_source_file, generted_header_filename, name),
